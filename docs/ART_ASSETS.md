...
All packages are original Quiet Arcade work. Raster covers are reproducible with `npm run art:memory`, `npm run art:word`, and `npm run art:color`; gameplay screenshots are captures of the actual game DOM. JPEG social cards are used for Open Graph, Twitter, and structured data.
