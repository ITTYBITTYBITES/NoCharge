...
| `/arcade/` | Compare ten games by genre, session, controls, and guides; solo and Pass &amp; Play groups are sectioned, not filtered | Open a game or guide | Registry and game pages | Distinct from collections because it is the full catalog | Existing editorial notes retained; no filters or tags; split trigger documented in `PASS_AND_PLAY.md` and deliberately not implemented |
