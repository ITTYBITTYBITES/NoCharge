...
**Status:** every recommendation above was implemented in this PR (see §9); R14 is the only one whose mechanics changed — `Astro` has no `{#anchor}` heading syntax (tested; the literal braces would render), so the numeric ids were removed at the source instead of remapped. R9 ended up taking both options: the copy was softened at fix time, and the six Pass & Play guides were then added, restoring the full claim.
...
| Content greps over `dist/` | About + Media "eleven solo titles" / "seventeen original browser games"; "Last reviewed: August 24, 2026" (accessibility); "Every game has a field guide" (/arcade/); "Field guides for every game in the arcade" (/guides/) (headings restored after the six Pass & Play guides landed); eleven-game clear dialog with the test-asserted consent sentence verbatim; both new changelog slugs present; named progress keys + `nocharge:passplay:match:*` in the saves-scores article; **zero** `{#…}` brace artifacts and **zero** digit-leading ids in built HTML |
...
| F-M10 | `/arcade/` guide heading → "Each solo game has a field guide."; `/guides/` → "Field guides for the solo arcade games". (The "full guide" line inside `/arcade/`'s solo-only section is accurate there and was left.) **Follow-up (same PR):** all six missing Pass & Play guides were then added (`tic-tac-toe`, `dots-and-boxes`, `four-in-a-row`, `reversi`, `last-token`, `pass-the-picture` — written against the shipped engines, R9's second option), so the headings were restored to the full claim: "Every game has a field guide." (/arcade/) and "Field guides for every game in the arcade" (/guides/) |
