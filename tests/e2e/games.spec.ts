...
test('Color Flip exposes four round-color buttons after Start', async ({ page }) => {
  ...
  for (const [name, shortcut] of choices) {
    const button = page.getByRole('button', { name: `Pick ${name}`, exact: true });
  ...
  await expect(page.locator('[data-cf="round-picker"]')).toBeHidden();
});

test('Color Flip G, B, A, and R shortcuts pick a round color', async ({ page }) => {
  await page.goto('/games/color-flip/');
  await page.getByRole('button', { name: 'Start' }).click();
