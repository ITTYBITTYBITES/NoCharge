...
describe('My Arcade local dashboard', () => {
  ...
  it('reads only documented NoCharge keys and never writes the derived model back', () => {
    ...
    const allowedReads = new Set([
      ...
      COLOR_TURN_HIGH,
      BEACON_PROGRESS_KEY,
      'nocharge:klondike:games-won',
      'nocharge:klondike:best-moves',
      'nocharge:freecell:games-won',
...
describe('Clear Game Data source of truth', () => {
  it('is the single allowlist shared by Privacy and My Arcade', () => {
    expect(CLEARABLE_GAME_DATA_KEYS).toEqual([
      ...
      'nocharge:minesweeper:games-won',
      'nocharge:minesweeper:best-time',
      'nocharge:hangman:games-solved',
      'nocharge:lights-out:puzzles-solved',
      'nocharge:lights-out:best-moves',
      'nocharge:simon:best-length',
      'nocharge:daily:word-loom:streak',
      'nocharge:daily:word-loom:solved',
      'nocharge:sudoku9:current-puzzle',
      'nocharge:sudoku9:puzzles-solved',
      ...
      'nocharge:pref:minesweeper-last-size',
      'nocharge:pref:hangman-last-theme',
      'nocharge:pref:simon-calm',
      RECENTLY_PLAYED_KEY,
