/**
 * Word Tile Rush pure engine logic.
 */
...
export interface WordTileRushState {
  grid: Cell[][];
  path: Position[];
  score: number;
  alive: boolean;
...
export function randomLetter(rng: () => number = Math.random): string {
...
export function createInitialGrid(rng: () => number = Math.random): Cell[][] {
  ...
  for (let r = ROWS - 3; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      if (rng() < 0.7) {
        grid[r]![c]!.letter = randomLetter(rng);
      }
    }
  }
  return grid;
...
export function addPositionToPath(
  grid: Cell[][],
  currentPath: Position[],
  pos: Position,
): { path: Position[]; changed: boolean; backtracked: boolean } {
  if (pos.r < 0 || pos.r >= ROWS || pos.c < 0 || pos.c >= COLS) {
    return { path: currentPath, changed: false, backtracked: false };
  }
  if (!grid[pos.r]?.[pos.c]?.letter) {
    return { path: currentPath, changed: false, backtracked: false };
  }

  const last = currentPath[currentPath.length - 1];
  if (last && last.r === pos.r && last.c === pos.c) {
    return { path: currentPath, changed: false, backtracked: false };
  }
  ...
  if (currentPath.length >= 2) {
    ...
    if (prev.r === pos.r && prev.c === pos.c) {
      ...
      return { path: newPath, changed: true, backtracked: true };
    }
  }

  // Cannot reuse an already visited cell
  if (currentPath.some((p) => p.r === pos.r && p.c === pos.c)) {
    return { path: currentPath, changed: false, backtracked: false };
  }
  ...
  return { path: [...currentPath, pos], changed: true, backtracked: false };
}

export function isValidWord(word: string): boolean {
  const lower = word.toLowerCase();
  return lower.length >= 3 && WORDS.has(lower);
}
...
export function submitPath(grid: Cell[][], path: Position[]): SubmitWordResult {
  const word = path.map((p) => grid[p.r]?.[p.c]?.letter ?? '').join('');
  if (!isValidWord(word)) {
    return {
      valid: false,
      word,
  ...
  const newGrid: Cell[][] = grid.map((row) =>
    row.map((cell) => ({ letter: cell.letter, selected: false })),
  );

  // Remove submitted letters
  for (const p of path) {
    if (newGrid[p.r]?.[p.c]) {
      newGrid[p.r]![p.c]!.letter = null;
    }
  }

  // Collapse columns down (gravity)
  for (let c = 0; c < COLS; c++) {
    const letters: string[] = [];
    for (let r = 0; r < ROWS; r++) {
      const l = newGrid[r]![c]!.letter;
      if (l) letters.push(l);
      newGrid[r]![c]!.letter = null;
    }
    let r = ROWS - 1;
    for (let i = letters.length - 1; i >= 0; i--) {
      newGrid[r]![c]!.letter = letters[i]!;
      r--;
    }
  }
...
export function dropNewRow(
  grid: Cell[][],
  rng: () => number = Math.random,
): DropRowResult {
  // If top row (row 0) has any letter, game over!
  if (grid[0]!.some((cell) => cell.letter !== null)) {
    return { gameOver: true, newGrid: grid };
  }
  ...
  // Shift rows up: row r gets letter from row r+1
  for (let r = 0; r < ROWS - 1; r++) {
    for (let c = 0; c < COLS; c++) {
      newGrid[r]![c]!.letter = grid[r + 1]![c]!.letter;
    }
  }

  // Fill new bottom row
  for (let c = 0; c < COLS; c++) {
    newGrid[ROWS - 1]![c]!.letter = rng() < 0.85 ? randomLetter(rng) : null;
  }

  return { gameOver: false, newGrid };
}
