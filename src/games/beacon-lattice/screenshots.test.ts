...
describe('gameplay screenshots', () => {
  test('art generator does not write gameplay screenshots', () => {
    const source = readFileSync('scripts/generate-beacon-lattice-art.mjs', 'utf8');
    expect(source).not.toMatch(/screenshot-desktop|screenshot-mobile/);
  });

  test('the mounted game does not expose a production debug seam', () => {
    const source = readFileSync('src/games/beacon-lattice/main.ts', 'utf8');
    expect(source).not.toContain('__NOCHARGE_BEACON_LATTICE_TEST__');
  });
